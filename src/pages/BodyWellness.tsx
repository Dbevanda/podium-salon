
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import { Link } from "react-router-dom";
import { ArrowLeft } from "lucide-react";

const BodyWellness = () => {
  const ingredients = [
    "Essential Oils Blend",
    "Dead Sea Minerals",
    "Organic Shea Butter",
    "Aromatherapy Extracts",
    "Marine Collagen",
    "Vitamin E Complex"
  ];

  return (
    <div className="min-h-screen bg-white">
      <Header />
      <main className="pt-32">
        <section className="py-20 bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <Link to="/treatments" className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-8">
              <ArrowLeft size={20} className="mr-2" />
              Back to Treatments
            </Link>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div>
                <img 
                  src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=800&h=600"
                  alt="Body Wellness Treatment"
                  className="w-full h-96 object-cover rounded-lg shadow-lg"
                />
              </div>
              
              <div>
                <h1 className="text-3xl md:text-4xl font-light text-gray-900 mb-6">
                  Body Wellness
                </h1>
                
                <div className="mb-8">
                  <h2 className="text-2xl font-medium text-gray-900 mb-4">Description</h2>
                  <p className="text-gray-600 leading-relaxed mb-4">
                    Experience complete body rejuvenation with our holistic wellness treatment that combines therapeutic massage, nourishing body wraps, and premium skincare.
                  </p>
                  <p className="text-gray-600 leading-relaxed">
                    This full-body treatment promotes circulation, reduces tension, and deeply moisturizes your skin for a complete wellness experience.
                  </p>
                </div>
                
                <div className="mb-8">
                  <h2 className="text-2xl font-medium text-gray-900 mb-4">Price</h2>
                  <p className="text-3xl font-semibold text-gray-900">$200</p>
                  <p className="text-gray-600">per session</p>
                </div>
                
                <div>
                  <h2 className="text-2xl font-medium text-gray-900 mb-4">Ingredients</h2>
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {ingredients.map((ingredient, index) => (
                      <li key={index} className="text-gray-600 flex items-center">
                        <span className="w-2 h-2 bg-gray-400 rounded-full mr-3"></span>
                        {ingredient}
                      </li>
                    ))}
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </div>
  );
};

export default BodyWellness;
